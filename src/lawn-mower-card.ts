...
export class LawnMowerCard extends LitElement {
  ...
  static getStubConfig(_: unknown, entities: string[]) {
    const [mowerEntity] = entities.filter((eid) =>
  ...
  get batteryEntity(): LawnMowerBatteryEntity | null {
    const batteryEntityId = this.config.battery_entity;
    if (!this.hass || !batteryEntityId) {
      return null;
    }
    return (
      (this.hass.states[batteryEntityId] as LawnMowerBatteryEntity) ?? null
    );
  }
  ...
  private handleMowerAction(
    action: string,
    params: LawnMowerActionParams = { request: true },
  ) {
    return () => {
      if (!this.config.actions[action]) {
        return this.callMowerService(params.defaultService || action, params);
      }
  ...
  private renderName(): Template {
    ...
    return html` <div class="mower-name">${friendly_name}</div> `;
  }
  ...
  private renderToolbar(state: LawnMowerEntityState): Template {
    ...
    switch (state) {
      case 'mowing': {
        return html`
          <div class="toolbar">
            <paper-button @click="${this.handleMowerAction('pause')}">
              <ha-icon icon="hass:pause"></ha-icon>
              ${localize('common.pause')}
            </paper-button>
            <paper-button @click="${this.handleMowerAction('dock')}">
      ...
      case 'paused': {
        return html`
          <div class="toolbar">
            <paper-button
              @click="${this.handleMowerAction('resume', {
        ...
        return html`
          <div class="toolbar">
            <ha-icon-button
              ...
              @click="${this.handleMowerAction('start_mowing', {
                defaultService: 'start_mowing',
                request: true,
              })}"
              ><ha-icon icon="hass:play"></ha-icon>
            ...
            <ha-icon-button
              ...
              @click="${this.handleMowerAction('dock')}"
              ><ha-icon icon="hass:home-map-marker"></ha-icon>
            </ha-icon-button>
